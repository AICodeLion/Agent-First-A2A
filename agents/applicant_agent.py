"""
agents/applicant_agent.py — 申请者用户在自己设备上运行这个脚本
...
def main():
    parser = argparse.ArgumentParser(
        description="申请者 Agent — 在自己设备上启动，等待 matchmaker 连接",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 使用 OpenClaw 默认路径
  python3 agents/applicant_agent.py --name "ddd"
  ...
  python3 agents/applicant_agent.py --name "ddd" \\
    ...
    agent    = BaseA2AAgent(
        name        = args.name,
        role_key    = scenario["role_B"],   # 通用：从场景配置读，不再硬编码
        scenario    = scenario,
        soul_path   = args.soul,
        user_path   = args.user,
